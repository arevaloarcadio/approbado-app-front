<template>
  ...
  <ion-row  >
   <ion-col size="8">
    <p style="margin-left: 22px;font-family: Segoe UI;font-style: normal;font-weight: 600;font-size: 18px;line-height: 22px;color: #000000;">Editar debate</p>
  </ion-col>
     ...
     <div style="display: flex;justify-content: center;margin-top: 8px;">
      ...
      <div style="display: flex;justify-content: center;margin-top: 8px;">
        ...
        <div  v-if="selected_tags.length == 0"  class="input-icons"> 
          <input type="text" name="title" class="input-form input-field" style="width :312px;   padding-bottom: 2px;" placeholder="Escoge una etiqueta" v-model="tag" > 
      ...
      <div style="display: flex;justify-content: center;margin-top: 36px;">
       <button style="width: 160px;height: 48px;margin-left: -153px;"  class="button-primary font-button" @click="editForum" >Publicar</button>
    </div>
  </ion-content>
</template>
...
export default defineComponent({
  ...
  mounted(){
    this.forum_id = this.$route.params.forum_id
    this.getForum()
    this.getTrivias()
    this.getTags()
  },
  methods:{
    ...
    getForum(){
      axios
      .get("/forums/"+this.forum_id)
      .then(res => {
        let forum = res.data;
        this.title = forum.message
        this.description = forum.summary
        this.trivia = forum.trivia.name
        this.trivia_id = forum.trivia.id;
        
        forum.categories.forEach(tag =>{
          this.tag_ids.push(tag.id);
          this.selected_tags.push(tag) 
        })

        if (forum.categories.length > 1) {
    ...
    async openSelectTag(Event) {
      console.log(this.tag_ids.length )
      ...
      popover.onDidDismiss().then((data) => {
        ...
        if(this.selected_tags != 0){
          ...
          this.tag_ids.forEach((delete_tag,key) =>{
            if(delete_tag == tag.id){
              this.tag_ids.splice(key, 1);
            }
          })
